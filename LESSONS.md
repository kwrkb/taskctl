...
## 2026-09-16: 未知の短縮フラグ判定では負の結果コードを除外する
- 却下した案: 2文字以上で `-` から始まる引数を、コマンドによらず未知フラグとして拒否する。
- 決め手: PR #16 で `taskctl explain -2147024891 --json` が「不明なフラグ」で終了コード1になり、同値の `0x80070005` は終了コード0だった。引数解析を通す回帰テストでも、負の10進数・int32最小値・負の16進数の4ケースが同じ理由で失敗した。`explain` の数値で始まる負数は位置引数として渡し、既存の結果コード解析で形式と範囲を検証する。
- 覆す条件: 符号付き結果コードの入力を廃止する仕様変更が承認された場合。
