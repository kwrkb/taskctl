...
- **git user.name をリポジトリローカルで `kwrkb` に変更**: グローバル設定はシステム実名（`yugosasaki`）のため、public 化した場合に実名がコミット履歴へ漏れるリスクがあった。グローバルは触らず、このリポジトリのみ上書き。email は `dev@kwrkb.com`（エイリアス）でそのまま。
