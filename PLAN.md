...
- 2026-07-16: **v2 レビュー対応**（3観点の自己レビュー + PR bot レビュー Gemini/Codex）。要修正: 移植時に case-sensitive 化していた比較 2 件（サービスアカウント判定・プロファイル変数検出）、取得層の堅牢化（PowerShell 不在時の捕捉・120秒タイムアウト・stdout/stderr ドレイン）、不正 XML の例外変換、クリーンチェックアウトでビルド不能だった埋込 JSON の MSBuild 生成ターゲット追加。さらに doctor テストを `DiagnosisContext` 注入でヘルメティック化し、explain --json / CliArgs / LocaleResolver / 取得層全体失敗のテストを補完。xUnit 157 テストパス（3回連続で安定確認）、v1 Pester 185 テストパスも維持
