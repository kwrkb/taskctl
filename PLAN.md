...
- [ ] `git init` + `.gitignore` + 初回コミット（VISION.md / result-codes.yaml / PLAN.md）
- [ ] `gh repo create kwrkb/taskctl --private` + push
- [ ] ディレクトリ骨格と空モジュール（`Import-Module` が通るだけの状態）
- [ ] Pester v5 実行環境の確認（`Invoke-Pester` が空テストで通る）
