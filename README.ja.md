...
**v2 を使ってください。** v1 は今も動作し、テストも CI で緑を維持していますが、
...
1. [Releases ページ](https://github.com/kwrkb/taskctl/releases/latest) から
   `taskctl-vX.Y.Z-win-x64.zip` をダウンロード
2. 任意の場所に展開（`taskctl.exe` 単一ファイル）
3. `taskctl.exe doctor` を実行

`PATH` の通った場所に置けば `taskctl doctor` でも呼べます。NativeAOT の単一 exe のため、
.NET ランタイムのインストールは不要です（取得層のみ実行時に `powershell` を内部で呼びます）。
